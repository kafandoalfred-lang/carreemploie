...
exports.handler = async (event, context) => {
  ...
  // Permettre les requêtes de pré-vérification CORS (CORS preflight)
  ...
  try {
    const { email, jobTitle, company, location, source, description, url, fullname } = JSON.parse(event.body);
    ...
    if (!RESEND_API_KEY) {
      return {
        ...
        body: JSON.stringify({ error: 'Resend API Key is not configured on Netlify environment variables.' })
      };
    }
